...
export function AIDiscussPage() {
  const { scripts, loadScripts, addScript } = useScriptStore();
  ...
  const handleSend = async () => {
    ...
    try {
      const result = await window.electronAPI.discussSettings(
        activeConfigId, 'script', getFields(),
  ...
  const handleExtractChars = async () => {
    ...
    try {
      ...
      if (result.reply) {
        ...
        if (match) {
          try { setDetectedChars(JSON.parse(match[0])); } catch { /* bad format */ }
        } else {
          setMessages((prev) => [...prev, { role: 'assistant', content: '❌ 未检测到角色信息' }]);
        }
      }
    } catch { /* ignore */ }
  ...
  const handleAddChar = async (char: any) => {
    ...
    try {
      await window.electronAPI.createCharacter({
        id: generateId(), scriptId: targetScriptId,
        name: char.name || '未命名', personality: char.personality || '',
        background: char.background || '', speakingStyle: char.speakingStyle || '',
        appearance: char.appearance || '', avatar: '', createdAt: Date.now(),
      } as any);
  ...
  "data": "其他设定（势力、等级、货币等，30-80字）"
    ...
    try {
      ...
      if (result.reply) {
        ...
        if (match) {
          let data: any;
          try { data = JSON.parse(match[0]); } catch { setMessages([...messages, { role: 'assistant', content: '❌ AI 返回格式异常，请重试' }]); setGenerating(false); return; }
          const now = Date.now();
          const script = await addScript({
            id: generateId(),
            title: data.title || targetTitle || 'AI 生成的剧本',
            ...
            extraData: {
              ...
              coreCheat: '', ageRule: '', narrativeMode: 'mode3', strictMode: 'strict',
              workflowMode: 'guided', recapMode: 'N', periodicSummary: 'O', ruleSelfCheck: 'Y',
            } as any,
            createdAt: now,
            updatedAt: now,
          });
          setTargetScriptId(script.id);
          selectScript(script.id);
          setMessages([...messages, { role: 'assistant', content: `✅ 剧本「${script.title}」已创建！可在剧本管理中查看和编辑。` }]);
        } else {
          setMessages([...messages, { role: 'assistant', content: '❌ 无法解析生成的JSON，请重试' }]);
        }
      }
    } catch (err: any) {
    ...
    <div className="flex-1 flex flex-col h-full">
      ...
      <div className="flex-shrink-0 bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex gap-2 ml-4">
          <select value={targetScriptId} onChange={(e) => { setTargetScriptId(e.target.value); setMessages([]); }}
          ...
          {!targetScriptId && (
            <input value={targetTitle} onChange={(e) => setTargetTitle(e.target.value)}
              className="bg-gray-800 border border-gray-700 rounded-lg px-2 py-1 text-xs text-gray-200 w-40"
              placeholder="剧本标题（可选）" />
          )}
        </div>
        ...
        <div className="ml-auto flex gap-2">
          {configs.map((c) => (
            <button key={c.id} onClick={() => useConfigStore.getState().setActiveConfig(c.id)}
              className={`px-2 py-1 text-xs rounded transition-colors ${activeConfigId === c.id ? 'bg-purple-900/50 text-purple-300' : 'bg-gray-700 text-gray-400 hover:bg-gray-600'}`}>
              {c.name}
            </button>
          ))}
        </div>
      ...
      {selectedScript && (
        <div className="flex-shrink-0 bg-gray-800/50 px-4 py-2 border-b border-gray-800 text-xs text-gray-500">
          当前讨论剧本：<span className="text-gray-300">{selectedScript.title}</span>
          · 世界观：{selectedScript.worldSetting?.slice(0, 40) || '未设定'}
        </div>
      )}
        ...
        <div className="max-w-3xl mx-auto space-y-3">
          {messages.length === 0 && (
            <div className="text-center text-gray-600 text-sm py-16">
              ...
              <p>在这里和 AI 讨论你的剧本创意</p>
              <p className="text-xs mt-2">AI 会按照十二步工作流引导你完善设定</p>
              {!activeConfigId && <p className="text-red-400 mt-2">请先在顶部选择一个 AI 配置</p>}
            </div>
          )}
          {messages.map((m, i) => (
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-gray-800 rounded-xl px-4 py-2.5 text-sm text-gray-400 animate-pulse">AI 思考中...</div>
            </div>
          )}
          <div ref={endRef} />
        </div>
      ...
      {detectedChars.length > 0 && targetScriptId && (
        <div className="flex-shrink-0 bg-gray-800/50 border-t border-gray-700 px-4 py-3">
          <div className="max-w-3xl mx-auto">
            <div className="text-xs text-gray-400 mb-2">👥 检测到 {detectedChars.length} 个角色，点击添加：</div>
            <div className="flex gap-2 flex-wrap">
              {detectedChars.map((c, i) => (
                <button key={i} onClick={() => handleAddChar(c)}
                  className="px-3 py-1.5 text-xs bg-gray-700 hover:bg-purple-900/50 border border-gray-600 hover:border-purple-500 text-gray-300 hover:text-purple-300 rounded-lg transition-colors flex items-center gap-1">
                  + {c.name}
                </button>
              ))}
            </div>
      ...
      <div className="flex-shrink-0 bg-gray-900 border-t border-gray-800 p-4">
        <div className="max-w-3xl mx-auto flex gap-3">
          ...
          <button onClick={handleExtractChars} disabled={extracting || messages.length === 0 || !activeConfigId}
            className="px-4 py-3 bg-blue-700 hover:bg-blue-600 disabled:bg-gray-700 disabled:text-gray-500 text-white rounded-xl text-sm font-medium transition-colors flex-shrink-0">
            {extracting ? '⏳' : '👥 提取角色'}
          </button>
          <button onClick={handleGenerate} disabled={generating || messages.length === 0 || !activeConfigId}
            className="px-4 py-3 bg-green-700 hover:bg-green-600 disabled:bg-gray-700 disabled:text-gray-500 text-white rounded-xl text-sm font-medium transition-colors flex-shrink-0">
            {generating ? '⏳' : '📝 生成剧本'}
          </button>
          <button onClick={handleSend} disabled={!input.trim() || loading || !activeConfigId}
