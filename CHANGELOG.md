...
### 🆕 新功能
- **📤 单剧本导出/导入**：每个剧本卡片独立导出+角色，跨设备导入合并模式
- **🔤 文字大小 5 档**：特小/小/中/大/特大 (12/14/16/18/20px)，全局生效持久化
- **📋 任务详情展开**：点击任务卡片展开 AI 生成的进度详情，面板内切换状态
- **👻 支线仅触发后显示**：pending 支线隐藏，触发后自动出现在面板中

### ✨ 改进
- ChatInput/ChatHeader/ChatMessages 用 React.memo 减少不必要重渲染
- 消息截断改为动态 token 计算（替代硬编码 100 条）
- 搜索消息高亮显示匹配文本
- 导入按钮加 loading 状态（⏳ 导入中...）
- CharacterCompendium 持久化加 800ms 防抖

### 🔧 修复
- 删除剧本/角色时级联清理关联对话和消息数据
- 分支对话复制最新 system prompt（含动态修改的回复长度等）
- QuestPanel useEffect 补 conversationId 依赖
- 统一 safeJsonParse 工具函数
