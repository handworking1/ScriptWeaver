...
- **🌍 世界介绍生成中指示器**：进入世界模式后显示「🌍 正在生成世界介绍...」加载条，AI 生成完成后自动消失
- **📖 角色图鉴增强**：侧面板支持手动编辑 + AI 自动分析，好感度滑块 + 7 级情感标签

### ✨ 改进
- **Token 统计分离**：全局累计 token + 当前会话 token 分别显示
- **多标签完善**：关闭活跃标签自动回退到其他标签，关闭最后一个标签回到设置页
...
- 角色图鉴闭包陈旧：快速连续编辑不再覆盖数据
- 角色图鉴 setTimeout 清理：组件卸载后不再调用 setState
- 世界介绍卸载保护：mountedRef 防止组件卸载后的异步 setState
- 世界模式启动空白期：加载指示器覆盖 loadMessages 等异步等待

### 🏗 架构
- **chat:send 事件化**：从 `ipcMain.handle`（阻塞主进程）改为 `ipcMain.on`（纯事件模式），SSE 长时间流不再阻塞其他 IPC 请求
- **导入数据字段级校验**：导入前逐行校验必填字段 + 类型 + role 枚举 + NaN 检测，损坏 JSON 给出具体错误定位
- **种子模板外部化**：7 个提示词模板提取到 `seedTemplates.ts`，便于多语言支持和迭代
- **退出前强制刷盘**：`app.on('before-quit')` 调用 `saveDbSync()`，防止丢失最后 500ms 的数据库写入
- **连续快速发送保护**：新请求自动中止旧请求的 AbortController
- **注释双语化**：所有新功能带中英文双语注释
