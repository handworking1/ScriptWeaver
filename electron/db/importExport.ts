...
export function importAllData(data: { scripts: any[]; characters: any[]; conversations: any[]; messages: any[]; aiConfigs: any[]; promptTemplates?: any[] }) {
  ...
  for (const c of data.conversations) d.run('INSERT INTO conversations (id, script_id, character_id, parent_id, title, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [c.id, c.script_id, c.character_id || null, c.parent_id ?? null, c.title ?? '', c.created_at, c.updated_at]);
